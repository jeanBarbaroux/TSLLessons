...
import {Fn, min, mx_noise_float, mx_noise_vec3, positionLocal, rotate, time, uv, vec2, vec3} from "three/tsl";
 ...
 */
{
    ...
    // opacity
    const smoke = mx_noise_float(uv().mul(vec2(3, 2)).sub(vec2(0, time.mul(0.2))))
    const edgeFade = min(
        uv().y.oneMinus(),
        uv().y.mul(10),
        uv().x.mul(5),
    ...
    material.opacityNode = edgeFade.mul(smoke).clamp(0, 1)
